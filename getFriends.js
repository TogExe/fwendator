...
async function gl(e, n) {
  for (let t in [fp, m, s] = [{}, Math.floor(e.length % 3600 / 60), Math.floor(e.length % 60)], console.log(`⏱ This will take about ${(m > 0 ? m + (1 == m ? " minute and " : " minutes and ") : "") + (s > 0 ? s + (1 == s ? " second" : " seconds") : "")}`), e) {
    const user = e[t];
